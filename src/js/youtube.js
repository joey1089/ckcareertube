const API_KEY = "AIzaSyCeVA8fAUPsTY2l5egTj2QrwBkS-zYj618";
...
const App = {
  ...
  async getVideos(ids) {
    ...
    const res = await fetch(
      `https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&key=${API_KEY}&id=${idsParam}`
    );

    const data = await res.json();
    const channelIds = [...new Set(data.items.map((i) => i.snippet.channelId))];
    const channelsRes = await fetch(
      `https://www.googleapis.com/youtube/v3/channels?part=snippet,contentDetails,statistics&key=${API_KEY}&id=${channelIds.join(
        ","
      )}`
    );
  ...
  createTemplate(video) {
    ...
    video-container
    ">
    <a class="row video-content" href="/video/index.html?id=${video.id}">
      <div class="video">
